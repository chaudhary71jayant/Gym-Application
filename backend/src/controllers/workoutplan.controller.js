...
const canAccessMember = async (member, user) => {
    ...
    if (["admin", "superAdmin"].includes(user.role) || memberUserId === user.id.toString()
    ){
        return true;
    }
    ...
    return (
        user.role === "trainer" &&
        Boolean(
        await Trainer.findOne({ user: user.id, assignedMembers: member._id }),
        )
    );
};
  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message: "You can create workout plans only for assigned members",
        });
    }
    ...
    await Notification.create({
      ...
      message: `Your trainer assigned a new workout plan: ${title}`,
      link: `/workout-plans/${workoutPlan._id}`,
    });
...
const getMemberWorkouts = async (req, res, next) => {
  try {
    ...
    if (!(await canAccessMember(member, req.user)))
      return res.status(403).json({ success: false, message: "Access denied" });
    const workoutPlans = await WorkoutPlan.find({ assignedTo: member._id })
      .populate({
        path: "createdBy",
    ...
    res
      .status(200)
      .json({ success: true, count: workoutPlans.length, workoutPlans });
  } catch (error) {
    next(error);
  }
};
...
const getWorkoutPlanById = async (req, res, next) => {
  try {
    ...
    if (!(await canAccessMember(workoutPlan.assignedTo || workoutPlan.assignedTo._id , req.user)))
      return res.status(403).json({ success: false, message: "Access denied" });
  ...
  try {
    ...
    if (!workoutPlan)
      return res
        ...
        .json({ success: false, message: "Workout plan not found" });
    const trainer = await Trainer.findOne({ user: req.user.id });
    if (!trainer || !workoutPlan.createdBy.equals(trainer._id))
      return res
        .status(403)
        .json({
          success: false,
          message: "You can update only your own workout plans",
        });
    ...
    if (!isAdmin) {
      ...
      if (!trainer || !workoutPlan.createdBy.equals(trainer._id))
        return res
          ...
          .json({
            success: false,
            message: "You can delete only your own workout plans",
          });
    }
